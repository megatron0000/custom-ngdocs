 ...
 */
var htmlEscape = require('./dom.js').htmlEscape; // Useless
var globalID = 0; // Useless
var errorsJson; // Useless
...
function Doc(text, file, startLine, endLine, options) {
    ...
    //this.options.sourceLink = function(){};
    //this.options.editLink = function(){};
...
Doc.prototype = {
    ...
    parse: function() {
        ...
        var self = this;
        /**
         * Set this instance's properties (param, methods, return ...). Only inline ones
         */
        self.text.split(NEW_LINE).forEach(function(line) {
            // If we find things like @type, @this, @that, ...
            if ((match = line.match(/^\s*@(\w+)(\s+(.*))?/))) {
                /**
                 * match:
                 *  1: name
                 *  2: useless... (content with \s at left)
                 *  3: content on same line (will be pushed at var atText)
                 */
                flush(); // if we have existing name
                atName = match[1];
                atText = [];
                if (match[3]) atText.push(match[3].trimRight());
            }
            // If not @name, @this, @that, we are at content line
            else {
                if (atName) { // That is, ignore empty lines
                    atText.push(line);
                }
            }
        });
        flush();
        /**
         * Set this.shortName using defined this.name
         */
        ...
        /**
         * Set remaining members of this Doc instance
         */
        ...
        // To build documentation for one @name, at the moment
        // when we reach another @name (finish the first to start processing the other)
        function flush() {
            if (atName) {
                var text = trim(atText.join('\n')),
                    match;
                // @module myModule         (seems useless)
                if (atName == 'module') {
                ...
                // @param {Array} testList A list of tests
                else if (atName == 'param') {
                    /**
                     * match:
                     *  1: Type of the param (suffixed by "=" if param is optional)
                     *  2: Useless...
                     *  3: Name of the param, if not optional (optional when marked like [ testList=["easy", "hard"] ])
                     *  4: Name of the param, if optional
                     *  5: Default value of the param, if optional
                     *  6: Param description (can be multi-line)
                     */
                    ...
                    if (dotIdx > 0) {
                        ...
                        var p = self.param.filter(function(p) {
                ...
                // @returns {number} Score on the test
                else if (atName == 'returns' || atName == 'return') {
                    /**
                     * match:
                     *  1: Type of return
                     *  2: Description of return (can be multi-line)
                     */
                    match = text.match(/^\{([^}]+)\}\s+(.*)/);
                    if (!match) {
                ...
                // @requires $scope Angular object to interact with DOM
                else if (atName == 'requires') {
                    // @requires {@link $scope http:/angularblabla}
                    ...
                    // @requires anythingIncludingLinks description part with spaces at will
                    else {
                        match = text.match(/^([^\s]*)\s*([\S\s]*)/);
                        self.requires.push({
                            name: match[1],
                ...
                // @property {string} assessmentName The name of the assessment
                     ...
                     *  2: Name
                     *  3: Useless...
                     *  4: Description (can be multi-line)
                     */
                    match = text.match(/^\{(\S+)\}\s+(\S+)(\s+(.*))?/);
                    if (!match) {
                    ...
                    var property = new Doc({
                        type: match[1],
                        name: match[2],
                        shortName: match[2], //Inline property is a piece of cake
                        // markdown deals with links etc...
                        description: self.markdown(text.replace(match[0], match[4]))
                    });
                    self.properties.push(property);
                }
                // @eventType Type target possibly with spaces
    ...
    /**
     * Builds entire html page corresponding to this Doc instance
     */
    html: function() {
        var dom = new DOM(),
            self = this,
        ...
        // Output "Improve this doc" link (must redirect to source code)
        ...
        // Output "View source" link (must redirect to source code)
        ...
        dom.h(title(this),
            ...
            function() {
                ...
                /**
                 * notice(): helper function. Checks if dom[firstParam] is undefined. Does nothing and returns if it is
                 * Otherwise:
                 * Uses first param as class for a <fieldset> element
                 * This fieldset has content equal to second param
                 * Inside the fieldset, a <legend> with message (thir param) is put
                 */
                notice('deprecated', 'Deprecated API', self.deprecated);

                /**
                 * For  @ngdoc error 
                 *      @name errorNamespace:errorCode,
                 * 
                 * display error message taken from exports.errorJson[namespace][code]
                 * (although this export is not defined)
                 */
                if (self.ngdoc === 'error') {
                ...
                dom.h('Dependencies', self.requires, function(require) {
                    ...
                    // require específico (como "@require ui.router Serve para blabla")
                ...
                // Other elements (properties, methods, ...)
        ...
        /**
         * Collects html "id" attributes and <a> element "name" attributes
         * 
         */
        self.anchors = dom.anchors;

        /**
         * .toString() concatenates dom.out into one big HTML string
         */
        return dom.toString();
        ...
        if (params.length > 0) {
            ...
            function processParams(params) {
                for (var i = 0; i < params.length; i++) {
                    ...
                    /**
                     * Extract starting "(" and ending ")", if any
                     */
                    if (types[0] == '(') {
                    ...
                    /**
                     * types becomes array of possible types.
                     * (example: @type {Array<string|number>|string} becomes ["Array<string|number>"  ,  "string"])
                     */
                    ...
                    /**
                     * Generate appropriate <a href></a> for redirecting to declared type
                     */
                    for (var j = 0; j < types.length; j++) {
                        var type = types[j];

                        /**
                         * Check if type has Array<something> , like in "assessmentAnalysis.type:Array<RawAssessment>"
                         */
                        var isArrayType = [];
                        if (isArrayType = type.match("Array<(.+)>"))
                            dom.tag('a', {
                                // href must process something like assessmentAnalysis.type:RawAssesment
                                href: self.convertUrlToAbsolute(type.replace(/Array<.+>/, isArrayType[1])),
                                class: self.prepare_type_hint_class_name(isArrayType[0])
                            }, isArrayType[0]);
                        else
                            dom.tag('a', {
                                href: self.convertUrlToAbsolute(type),
                                class: self.prepare_type_hint_class_name(type)
                            }, type.replace(/^.+:/, ""));
    ...
    html_usage_method: function(dom) {
        var self = this;
        self.html_usage_property(dom);
    },
    ...
    html_usage_filter: function(dom) {
        var self = this;
        dom.h('Usage', function() {
            dom.h('No Binding do template HTML', function() {
                dom.tag('code', function() {
                    if (self.usage) {
                        dom.text(self.usage);
                    } else {
    ...
    html_usage_directiveInfo: function(dom) {
        ...
        if (list.length) {
            dom.h('Directive information', function() {
                dom.ul(list);
            });
        }
    },
        ...
        if (self.methods.length) {
            dom.div({
                ...
                dom.h('Methods', self.methods, function(method) {
                    // If we set sourceLink(file, line, codeline)
                    ...
                    //////////////////////////////
                    let privatePublicClass = (/@public/.test(method.text) ? 'public' : (/@private/.test(method.text) ? 'private' : 'nonSpecified'));
                    let deprecatedClass = /@deprecated/.test(method.text) ? 'deprecated' : "";
                    let vitorCustomClassObj = {
                        'class': privatePublicClass + " " + deprecatedClass
                    };
                    //////////////////////////////
                    // Observar que a callback é chamada com argumento 
                    ...
                    dom.h(method.shortName + '(' + signature.join(', ') + ')', method,
                        function(objMethod) {
                            ...
                            //dom.html(method.html());
                            objMethod.html_usage_parameters(dom);
                            self.html_usage_this(dom);
                            objMethod.html_usage_returns(dom);

                            dom.h('Example', objMethod.example, dom.html);
                        },
                        //////////////////////////////
                        vitorCustomClassObj
                        //////////////////////////////
        ...
        if (self.properties.length) {
            dom.div({
                ...
                dom.h('Properties', self.properties, function(property) {
                    //////////////////////////////
                    let privatePublicClass = (/@public/.test(property.text) ? 'public' : (/@private/.test(property.text) ? 'private' : 'nonSpecified'));
                    let deprecatedClass = (/@deprecated/.test(property.text) ? 'deprecated' : "");
                    let vitorCustomClassObj = {
                        'class': privatePublicClass + " " + deprecatedClass
                    };
                    //////////////////////////////
                    dom.h(property.shortName, function() {
                            ...
                            dom.html('<td>');
                            /**
                             * Links correctly to type of property (if internally declared)
                             */
                            var type = property.type;
                            //  Check if type is array like "modulename.type:Array<something>"
                            var isArrayType = [];
                            if (isArrayType = type.match("Array<(.+)>"))
                                dom.tag('a', {
                                    class: self.prepare_type_hint_class_name(isArrayType[0]),
                        ...
                        // Atenção: Precisa de 4 argumentos
                        undefined,
                        vitorCustomClassObj
                    );
                });
